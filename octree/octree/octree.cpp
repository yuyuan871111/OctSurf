...
void Octree::set_octree(const char* data, const int64_t sz) {               // qq: TODO: with batches, sz can exceed int range
  resize_octree(sz);
  memcpy(buffer_.data(), data, sz);
  this->set_cpu(buffer_.data());
}
void Octree::set_octree(const char* data, const int sz) {               // qq: TODO: with batches, sz can exceed int range
  resize_octree(sz);
  memcpy(buffer_.data(), data, sz);
  this->set_cpu(buffer_.data());
}
...
void Octree::resize_octree(const int64_t sz) {                             // qq: TODO: with batches, sz can exceed int range
  buffer_.resize(sz);
  this->set_cpu(buffer_.data());
}
void Octree::resize_octree(const int sz) {                             // qq: TODO: with batches, sz can exceed int range
  buffer_.resize(sz);
  this->set_cpu(buffer_.data());
}
...
bool Octree::read_octree(const string& filename) {
  ...
  std::cout << "size of octree info: " << sizeof(OctreeInfo) <<std::endl;
...
void Octree::serialize() {
  const int64_t sz = oct_info_.sizeof_octree();                                 // qq: TODO: need update to long or uintk
  buffer_.resize(sz, 0);
...
void Octree::serialize(Dtype* des, const vector<vector<Dtype> >& src, const int location) {         // qq: location to long ?
  if (location == -1) {
