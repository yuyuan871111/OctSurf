...
void memset_cpu(const int N, const Dtype alpha, Dtype* Y) {
  if (alpha == 0) {
    memset(Y, 0, sizeof(Dtype) * N);                            // qq: does memset work with larger value?
    return;
  }
  for (int i = 0; i < N; ++i) {
    Y[i] = alpha;
...
void octree_max_pool_cpu(Dtype* top_data, int top_h, int* mask,
  ...
  for (int c = 0; c < channel; ++c) {
    for (int h = 0; h < top_h; ++h) {
      int hb = 8 * h;                                       // qq: TODO: long: do not have to, just involve num_nodes
...
void octree_max_unpool_cpu(const Dtype* top_data, int top_h, const int* mask,
  ...
  memset_cpu(btm_h * channel, Dtype(0), btm_data);        // qq: TODO: long
...
void calc_neigh_cpu(int* neigh_split, const int* neigh,     // qq: here neigh_split is for current level d, neigh, children, node_num are for d-1.
  ...
  for (int i = 0; i < node_num; ++i) {                      // qq: for every node in d-1,
    int l0 = children[i];
    if (l0 == -1) continue;                                 // qq: if -1, means no child, empty
    const int* ngh0 = neigh + (i >> 3 << 6);                // qq: the children's index in level d
    const int* pi0 = parent + (i % 8) * 64;
    int* ngh1 = neigh_split + (l0 << 6);                    // qq: l0 could be 10 million, << 6 is times 64. might be too large when we have large batch size.
    for (int j = 0; j < 64; ++j) {
      ngh1[j] = -1;
      int k = ngh0[pi0[j]];
      if (k != -1) {
        int l1 = children[k];
        if (l1 != -1) {
          ngh1[j] = (l1 << 3) + dis[j];                     // qq: l1<<3 could 80 million with bs=1.  dis is small.
        }
      }
    }
  }
