...
void MergeOctrees::set_batch_info() {
  ...
  info_batch_.set_property(OctreeInfo::kNeigh, kNeighChannel, -1);  // qq: kNeighChannel is channel, -1 is location, means the for every octants
  // update nodenumber
  info_batch_.set_nnum(nnum_batch_.data());                         // qq: cumulative node number of the batch,
  info_batch_.set_nempty(nnum_nempty_batch_.data());
  info_batch_.set_nnum_cum();                                       // qq: default capacity is 0. set nnum_cum_ from d=0 to depth +2
  info_batch_.set_ptr_dis();                                        // qq: check
  // bool valid = info_batch.check_format(err_msg);
  // CHECK(valid) << err_msg;
}

void MergeOctrees::set_batch_parser(vector<char>& octree_out) {
  int64_t sz = info_batch_.sizeof_octree();       // qq: TODO: long sz
  ...
  for (int i = 0; i < batch_size_; ++i) {
    ...
    for (int d = depth_start; d < depth_ + 1; ++d) {
      ...
      for (int c = 0; c < feature_channel; c++) {
        float* des = octbatch_parser_.mutable_feature_cpu(d) +                         // qq: here ~240 million with bs=1.
