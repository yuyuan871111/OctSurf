...
void calc_neigh_gpu(int* neigh_split, const int* neigh,  const int* children,
  ...
  int n = node_num << 6; // node_num: the non_empty node number of parent layer  // qq:  2 million * 2^6 < 2billion.
...
void calc_neigh_gpu(int* neigh, const int depth, const int batch_size) {
  int thread_num = batch_size * (1 << 3 * depth + 3);                       // qq: TODO: long?
...
void generate_key_gpu(Dtype* key, const int depth, const int batch_size) {
  int thread_num = batch_size * (1 << 3 * depth);                           // qq: TODO: 1 << 3 * depth is much more than int
