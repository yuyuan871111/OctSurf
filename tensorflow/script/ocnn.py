...
def conv3d(inputs, nout, kernel_size, stride, padding='SAME', data_format='channels_first'):
  return tf.layers.conv3d(inputs, nout, kernel_size=kernel_size, strides=stride,
            padding=padding, data_format=data_format, use_bias=False,
            kernel_initializer=tf.contrib.layers.xavier_initializer())
...
def conv3d_bn(inputs, nout, kernel_size, stride, training):
  conv = conv3d(inputs, nout, kernel_size, stride)
  # return conv
  return batch_norm(conv, training)
...
def conv3d_bn_relu(inputs, nout, kernel_size, stride, training):
  with tf.variable_scope('conv_bn_relu'):
    out = conv3d_bn(inputs, nout, kernel_size, stride, training)
    out = tf.nn.relu(out)
  return out
...
def max_pool3d(inputs, data_format='NCDHW'):
  return tf.nn.max_pool3d(inputs, ksize=[1,1,2,2,2], strides=[1,1,2,2,2],
      padding='SAME', data_format=data_format)

def avg_pool3d(inputs, data_format='NCDHW'):
  return tf.nn.avg_pool3d(inputs, ksize=[1,1,2,2,2], strides=[1,1,2,2,2],
      padding='SAME', data_format=data_format)
...
def grids_resblock(grids, num_out, stride, training, bottleneck=4):
  num_in = int(grids.shape[1])
  channelb = int(num_out / bottleneck)
  if stride == 2:
    grids = max_pool3d(grids)

  with tf.variable_scope("1x1x1_a"):
    block = conv3d_bn_relu(grids, channelb, kernel_size = 1, stride=1, training=training)

  with tf.variable_scope("3x3x3"):
    block = conv3d_bn_relu(block, channelb, kernel_size = 3, stride=1, training=training)

  with tf.variable_scope("1x1x1_b"):
    block = conv3d_bn_relu(block, num_out, kernel_size = 1, stride=1, training=training)


  if num_in != num_out:
    with tf.variable_scope("1x1x1_c"):
      block4 = conv3d_bn_relu(grids, num_out, kernel_size = 1, stride=1, training=training)
      return tf.nn.relu(block + block4)
  else:
    return tf.nn.relu(block + grids)
...
# QQ add:
def mse_loss(logit, label, num_class = 1):
  with tf.name_scope('mse_loss'):
    loss = tf.losses.mean_squared_error(label, logit)
  return loss
  pass

def loss_functions_reg(logit, label_gt, num_class, weight_decay, var_name, label_smoothing=0.0):
  with tf.name_scope('loss'):
    logit = tf.reshape(logit, [-1])
    loss = mse_loss(logit, label_gt, num_class)
    accu = loss
    regularizer = l2_regularizer(var_name, weight_decay)
  return [loss, accu, regularizer]
