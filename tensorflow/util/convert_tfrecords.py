...
parser.add_argument('--label_type', type=str, required=False, default='int',
                    help='If label is int for classification or float for regression')
shuffle_data = False
...
def write_data_to_tfrecords(file_dir, list_file, records_name, file_type, label_type = 'int'):
  [data, label, index] = get_data_label_pair(list_file, label_type)
  ...
  for i in range(len(data)):
    ...
    if label_type == 'int':
        ...
        feature = {file_type: _bytes_feature(octree_file),
                   'label': _float_feature(label[i]), # QQ: add float option
                   'index': _int64_feature(index[i]),
                   'filename': _bytes_feature(('%06d_%s' % (i, data[i])).encode('utf8'))}
...
def get_data_label_pair(list_file, label_tye = 'int'):
  file_list = []
  label_list = []
  with open(list_file) as f:
    for line in f:
      ...
      if label_tye == 'int':        # QQ: add float option
...
if __name__ == '__main__':
  ...
  write_data_to_tfrecords(args.file_dir, 
                          ...
                          args.records_name, 
                          args.file_type,
                          args.label_type)  # QQ: add float option
