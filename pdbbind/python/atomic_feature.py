# For OctSurfNet, get the atomic features for pocket and ligand.
...
# import pybel
# import openbabel
from openbabel import pybel             # openbabel: version 3
...
def get_cdk_vdwr_dic(file = '../../../python/Elements.java'): 
    """extract van der waal radius from cdk Element.java file"""
    cdk_vdwr_dic = {}
    with open(file, 'r') as f:
        lines = f.readlines()
        start = False
        for line in lines:
            if 'public enum Elements {' in line:
                start = True
                continue
            if start:
                ...
                if 'null' in radius:
                    ...
                    cdk_vdwr_dic[atomic_number] = float(radius)
                if 'Oganesson' in line:
                    break
            else:
                continue
    return cdk_vdwr_dic
cdk_vdwr_dic = get_cdk_vdwr_dic()
# print(cdk_vdwr_dic)
...
class Featurizer():
    ...
    def get_features(self, molecule, molcode=None):
        ...
        for i, atom in enumerate(molecule):
            ...
            # ['hyb', 'heavyvalence', 'heterovalence',
            #                                 'partialcharge']
            atom_attributes = []
            for prop in self.NAMED_PROPS:
                if prop == 'hyb':
                    atom_attributes.append(atom.hyb)
                if prop == 'heavyvalence':
                    atom_attributes.append(atom.heavydegree)
                if prop == 'heterovalence':
                    atom_attributes.append(atom.heterodegree)
                if prop == 'partialcharge':
                    atom_attributes.append(atom.partialcharge)
                if prop == 'radius':
                    temp = atom.atomicnum
                    # temp2 = ob.GetVdwRad(temp)
                    # if abs(temp2 - 1.1) < 0.01:
                    #     temp2 = 1.2
                    temp2 = cdk_vdwr_dic[temp]
                    atom_attributes.append(temp2)
                    pass
...
def write_feature_file(folder):
    contents = folder.split('/')
    id = contents[-1]
    ligand_file = folder + '/{}_ligand.mol2'.format(id)
    pocket_file = folder + '/{}_pocket.pdb'.format(id)

    ligand = next(pybel.readfile('mol2', ligand_file))
    featurizer = Featurizer()
    ligand_coords, ligand_feature = featurizer.get_features(ligand, molcode=0)
    with open('{}/{}_ligand_feature.txt'.format(folder,id), 'w') as f:
        for i in range(ligand_feature.shape[0]):
            # f.writelines('{} {}\n'.format(i+1, ligand_feature[i].tolist()))
            f.writelines('{}'.format(i+1))
            for j in ligand_coords[i]:
                f.writelines(' {}'.format(j))
            for j in ligand_feature[i]:
                f.writelines(' {}'.format(j))
            f.writelines('\n')

    pocket = next(pybel.readfile('pdb', pocket_file))
    featurizer = Featurizer()
    pocket_coords, pocket_feature = featurizer.get_features(pocket, molcode=1)
    with open('{}/{}_pocket_feature.txt'.format(folder, id), 'w') as f:
        for i in range(pocket_feature.shape[0]):
            # f.writelines('{} {}\n'.format(i+1, pocket_feature[i].tolist()))
            f.writelines('{}'.format(i+1))
            for j in pocket_coords[i]:
                f.writelines(' {}'.format(j))
            for j in pocket_feature[i]:
                f.writelines(' {}'.format(j))
            f.writelines('\n')
    pass
